...
if($_POST['submit']){
	...
	if($action == "add"){
		...
		if($result_list->num_rows){
			...
			$db->query($sql_update);
			if($db->affected_rows){
				$sql = "INSERT INTO `db_cutter_inout` (`inoutid`,`listid`,`dotype`,`form_number`,`dodate`,`quantity`,`employeeid`,`remark`,`dotime`) VALUES (NULL,'$listid`','I','$form_number','$dodate','$quantity','$employeeid','$remark','$dotime')";
				$db->query($sql);
				if($db->insert_id){
