...
$objActSheet->setTitle('物料异常入库记录'.date('Y-m-j')."-BY_JTL"); 
...
$objActSheet->setCellValue('G1', '入库数量');
...
$objActSheet->setCellValue('P1', '计划回厂日期');
$objActSheet->setCellValue('Q1', '差异');

$sql = $_SESSION['material_abnormal_entry']." ORDER BY `db_material_inout`.`inoutid` DESC";
$result = $db->query($sql);
if($result->num_rows){
	$i = 2;
	while($row = $result->fetch_assoc()){
		...
		$objActSheet->setCellValue('I'.$i, $row['inout_quantity']);
		$objActSheet->setCellValue('J'.$i, $row['unit_name_actual']);
		...
		$objActSheet->setCellValue('P'.$i, $row['plan_date']);
		$objActSheet->setCellValue('Q'.$i, $row['diff_date']);
		$i++;
	}
...
$m_strOutputExcelFileName = '物料异常入库记录_'.date('Y-m-j_H_i_s').".xls";
//输到文件   
