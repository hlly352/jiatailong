...
if($_POST['submit']){
	...
	if($action == "add"){
		...
		if($result_list->num_rows){
			...
			$db->query($sql_update);
			if($db->affected_rows){
				$sql = "INSERT INTO `db_material_inout` (`inoutid`,`dodate`,`dotype`,`form_number`,`quantity`,`taker`,`remark`,`listid`,`employeeid`,`dotime`) VALUES (NULL,'$dodate','O','$form_number','$quantity','$taker','$remark','$listid`','$employeeid','$dotime')";
				$db->query($sql);
				if($db->insert_id){
