...
$objActSheet->setTitle('刀具入库记录'.date('Y-m-j')."-BY_JTL"); 
...
$objActSheet->setCellValue('I1', '订单数量');
...
$objActSheet->setCellValue('N1', '供应商');
$objActSheet->setCellValue('O1', '入库日期');

$sql = $_SESSION['cutter_inout_list_in']." ORDER BY `db_cutter_inout`.`inoutid` DESC";
$result = $db->query($sql);
if($result->num_rows){
	$i = 2;
	while($row = $result->fetch_assoc()){
		...
		$objActSheet->setCellValue('I'.$i, $row['order_quantity']);
		$objActSheet->setCellValue('J'.$i, $row['in_quantity']);
		$objActSheet->setCellValue('K'.$i, '件');
		$objActSheet->setCellValue('L'.$i, $row['unit_price']);
		$objActSheet->setCellValue('M'.$i, $row['amount']);
		$objActSheet->setCellValue('N'.$i, $row['supplier_cname']);
		$objActSheet->setCellValue('O'.$i, $row['dodate']);
		$i++;
	}
...
$m_strOutputExcelFileName = '刀具入库记录_'.date('Y-m-j_H_i_s').".xls";
//输到文件   
