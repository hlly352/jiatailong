...
if($_POST['submit']){
	...
	if($action == "add"){
		...
		$dotime = fun_gettime();
		$sql = "INSERT INTO `db_godown_entry` (`entryid`,`entry_number`,`entry_date`,`employeeid`,`dotype`,`dotime`) VALUES (NULL,'$entry_number','$entry_date','$employeeid','O','$dotime')";
		$db->query($sql);
		if($db->insert_id){
			header("location:other_material_godown_entry.php");
		}
	}elseif($action == "del"){
		$array_id = $_POST['id'];
