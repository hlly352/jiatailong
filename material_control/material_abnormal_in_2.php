...
<div id="table_list">
  ...
  <table>
    ...
    //$order_sql = "SELECT group_concat(`db_material_order_list`.`listid`),SUM(`db_material_order_list`.`order_quantity`) AS `sum` FROM `db_material_order_list` INNER JOIN `db_material_order` ON `db_material_order`.`orderid` = `db_material_order_list`.`orderid` GROUP BY `db_material_order_list`.`orderid`";
  //   $order_sql = "SELECT `db_material_inout`.`inout_quantity` FROM `db_material_inout` WHERE `db_material_inout`.`listid` IN(SELECT group_concat(`db_material_order_list`.`listid`) FROM `db_material_order_list` INNER JOIN `db_material_order` ON `db_material_order`.`orderid` = `db_material_order_list`.`orderid` GROUP BY `db_material_order_list`.`orderid`)";
  $order_sql = "SELECT * FROM `db_material_inout` LEFT JOIN  `db_material_order_list` ON `db_material_inout`.`listid` = `db_material_inout`.`listid` WHERE `db_material_order_list`.`orderid` IN(SELECT `db_material_order_list`.`orderid` FROM `db_material_inout` LEFT JOIN `db_material_order_list` ON `db_material_inout`.`listid` = `db_material_order_list`.`listid` WHERE `db_material_order_list`.`order_quantity` != `db_material_inout`.`inout_quantity` GROUP BY `db_material_order_list`.`orderid`)";
    echo $order_sql;
  ...
  var_dump($info);
	?>
  <?php if($info != $row['inout_quantity']){ ?>
    <tr>
      ...
      <td>
        <?php echo $info - $row['inout_quantity'] ?>
      </td>
    </tr>
    <?php }} ?>
  </table>
