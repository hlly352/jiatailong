...
$objActSheet->setTitle('刀具出库记录'.date('Y-m-j')."-BY_JTL"); 
...
$objActSheet->setCellValue('B1', '申领编号');
...
$objActSheet->setCellValue('H1', '更换数量');
$objActSheet->setCellValue('I1', '单位');
$objActSheet->setCellValue('J1', '申领人');
$objActSheet->setCellValue('K1', '模具编号');
$objActSheet->setCellValue('L1', '出库日期');

$sql = $_SESSION['cutter_inout_list_out']." ORDER BY `db_cutter_inout`.`inoutid` DESC";
$result = $db->query($sql);
if($result->num_rows){
	$i = 2;
	while($row = $result->fetch_assoc()){
		...
		$objActSheet->setCellValue('H'.$i, $row['old_quantity']);
		$objActSheet->setCellValue('I'.$i, '件');
		$objActSheet->setCellValue('J'.$i, $row['employee_name']);
		$objActSheet->setCellValue('K'.$i, $row['mould_number']);
		$objActSheet->setCellValue('L'.$i, $row['dodate']);
		$i++;
	}
...
$m_strOutputExcelFileName = '刀具出库记录_'.date('Y-m-j_H_i_s').".xls";
//输到文件   
