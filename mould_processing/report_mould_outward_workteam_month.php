...
//workteamid+month
$sql_workteam_month = "SELECT `workteamid`,DATE_FORMAT(`order_date`,'%Y-%m') AS `month`,SUM(`cost`) AS `cost` FROM `db_mould_outward` WHERE DATE_FORMAT(`order_date`,'%Y') = '$year' AND `outward_status` = 1 GROUP BY `workteamid`,DATE_FORMAT(`order_date`,'%Y-%m')";
...
<div id="table_search">
  <h4>外协组别月报表</h4>
  <form action="" name="report_mould_outward_workteam_month" method="get">
    <table>
      <tr>
        <th>年份：</th>
