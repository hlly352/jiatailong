...
<?php
	...
	if($_FILES['file']['size'] > 106132773 || $_FILES['file']['size'] == 0){
		echo '文件超大，请重新上传<a href="design_out_edit.php?action=add&specification_id='.$specification_id.'">返回</a>';
		return false;
	}
	//上传文件
	if($_FILES['file']['name']){
	...
	//新建当前项目中没有的模具信息
		...
		foreach($email_employeeid as $employeeid){
			if(!empty($employeeid)){
				$str_show .= '(\''.$doc_type.'\',\''.$employeeid.'\',\''.$informationid.'\'),';
			}
		}
		$str_show = rtrim($str_show,',');
		$sql_show = "INSERT INTO `db_mould_data_show`(`data_name`,`employeeid`,`informationid`) VALUES $str_show";
		$db->query($sql_show);
		//获取邮件发送地址
		$address = array();
		...
		$subject = $mould_number.'的'.$array_processing_data[$doc_type].'发生更新';
		$body = $mould_number.'的'.$array_processing_data[$doc_type].'发生更新,请查看';
		send($send,$address,$subject,$body,$file_path);
	$db->query($sql);
	if($db->affected_rows){
		header('location:processing_data.php');
	}
}elseif($action == 'del'){
	$data = $_GET['data'];
