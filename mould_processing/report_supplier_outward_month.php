...
//supplierid+month
$sql_supplier_month = "SELECT `supplierid`,DATE_FORMAT(`order_date`,'%Y-%m') AS `month`,SUM(`cost`) AS `cost` FROM `db_mould_outward` WHERE DATE_FORMAT(`order_date`,'%Y') = '$year' AND `outward_status` = 1 GROUP BY `supplierid`,DATE_FORMAT(`order_date`,'%Y-%m')";
...
$sql_supplier = "SELECT `supplierid`,`supplier_cname` FROM `db_supplier` WHERE `supplierid` IN ($array_supplierid) ORDER BY `supplierid`";
...
<div id="table_search">
  <h4>外协供应商月报表</h4>
  <form action="" name="report_supplier_outward_month" method="get">
    <table>
      <tr>
        <th>年份：</th>
