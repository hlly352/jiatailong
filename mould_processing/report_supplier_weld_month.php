...
//supplierid+month
$sql_supplier_month = "SELECT `supplierid`,DATE_FORMAT(`order_date`,'%Y-%m') AS `month`,SUM(`cost`) AS `cost` FROM `db_mould_weld` WHERE DATE_FORMAT(`order_date`,'%Y') = '$year' AND `weld_status` = 1 GROUP BY `supplierid`,DATE_FORMAT(`order_date`,'%Y-%m')";
...
<div id="table_search">
  <h4>烧焊供应商月报表</h4>
  <form action="" name="report_supplier_weld_month" method="get">
    <table>
      <tr>
        <th>年份：</th>
