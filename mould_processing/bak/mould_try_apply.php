...
$sql = "SELECT `db_mould_try`.`tryid`,`db_mould_try`.`mould_size`,CONCAT('T',`db_mould_try`.`try_times`) AS `try_times`,`db_mould_try`.`tonnage`,`db_mould_try`.`molding_cycle`,`db_mould_try`.`plastic_material_color`,`db_mould_try`.`plastic_material_offer`,`db_mould_try`.`product_weight`,`db_mould_try`.`product_quantity`,`db_mould_try`.`material_weight`,`db_mould_try`.`plan_date`,`db_mould_try`.`employeeid`,`db_mould_try`.`approve_status`,`db_mould_try`.`try_status`,`db_mould`.`mould_number`,`db_mould`.`cavity_number`,`db_mould`.`difficulty_degree`,`db_mould`.`plastic_material`,`db_mould`.`assembler`,`db_mould_try_cause`.`try_causename`,`db_applyer`.`employee_name` AS `applyer_name`,`db_approver`.`employee_name` AS `approver_name` FROM `db_mould_try` INNER JOIN `db_mould` ON `db_mould`.`mouldid` = `db_mould_try`.`mouldid` INNER JOIN `db_mould_try_cause` ON `db_mould_try_cause`.`try_causeid` = `db_mould_try`.`try_causeid` INNER JOIN `db_employee` AS `db_applyer` ON `db_applyer`.`employeeid` = `db_mould_try`.`employeeid` INNER JOIN `db_employee` AS `db_approver` ON `db_approver`.`employeeid` = `db_mould_try`.`approver` $sqlwhere";
$result = $db->query($sql);
