  ...
  <form action="moulddo.php" name="mould_list" method="post">
    ...
    ?>
      <tr>
        ...
        <td>
          <?php echo shows($row,'part_report') ?>
        </td>
        <td>
          <?php echo shows($row,'product_report'); ?>
        </td>
        <td>
          <?php echo shows($row,'error_report'); ?>
        </td>
        <td><a href="<?php echo 'quality_data_edit.php?action=add&specification_id='.$row['mould_specification_id'].'&mouldid='.$row['mould_dataid']; ?>">更新</a></td>
      </tr>
      <?php } ?>
    </table>
   <!--  <div id="checkall">
