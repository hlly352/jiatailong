...
<?php
	...
	if($_FILES['file']['size'] > 106132773 || $_FILES['file']['size'] == 0){
		echo '文件超大，请重新上传<a href="quality_data_edit.php?action=add&specification_id='.$specification_id.'">返回</a>';
		return false;
	}
	//上传文件
	if($_FILES['file']['name']){
	...
	//新建当前项目中没有的模具信息
		...
		$subject = $mould_number.'的'.$array_quality_data[$doc_type].'发生更新';
		$body = $mould_number.'的'.$array_quality_data[$doc_type].'发生更新,请查看';
		send($send,$address,$subject,$body,$file_path);
	$db->query($sql);
	if($db->affected_rows){
		header('location:quality_data.php');
	}
}elseif($action == 'del'){
	$data = $_GET['data'];
