...
$cutterid = fun_check_int($_GET['cutterid']);
...
if($result_purchase->num_rows){
	...
	$sql_cutter = "SELECT `db_cutter_specification`.`typeid`,`db_mould_cutter`.`specificationid`,`db_cutter_hardness`.`texture`,`db_mould_cutter`.`hardnessid` FROM `db_mould_cutter` INNER JOIN `db_cutter_specification` ON `db_cutter_specification`.`specificationid` = `db_mould_cutter`.`specificationid` INNER JOIN `db_cutter_hardness` ON `db_cutter_hardness`.`hardnessid` = `db_mould_cutter`.`hardnessid` WHERE `db_mould_cutter`.`cutterid` = '$cutterid'";
	$result_cutter = $db->query($sql_cutter);
	if($result_cutter->num_rows){
		$array_cutter = $result_cutter->fetch_assoc();
		$cutter_typeid = $array_cutter['typeid'];
		$cutter_texture = $array_cutter['texture'];
		//规格
    ...
    <table>
      ...
      <tr>
        ...
        <td><select name="specificationid" id="specificationid">
			...
			?>
            <option value="<?php echo $row_cutter_specification['specificationid']; ?>"<?php if($row_cutter_specification['specificationid'] == $array_cutter['specificationid']) echo " selected=\"selected\""; ?>><?php echo $row_cutter_specification['specification']; ?></option>
            <?php
				}
			}
			?>
        ...
        <td><select name="hardnessid" id="hardnessid">
			...
			?>
            <option value="<?php echo $row_cutter_hardness['hardnessid']; ?>"<?php if($row_cutter_hardness['hardnessid'] == $array_cutter['hardnessid']) echo " selected=\"selected\""; ?>><?php echo $row_cutter_hardness['hardness']; ?></option>
            <?php
				}
			}
			?>
      ...
      <tr>
        ...
        <td><select name="brandid" id="brandid">
			...
			?>
            <option value="<?php echo $row_cutter_brand['brandid']; ?>"><?php echo $row_cutter_brand['brand']; ?></option>
            <?php
				}
			}
			?>
      ...
      <tr>
        ...
        <td colspan="7"><input type="submit" name="submit" id="submit_do" value="确定" class="button" />
          ...
          <input type="hidden" name="cutterid" value="<?php echo $cutterid; ?>" />
...
<div id="table_search">
  <h4>刀具数据</h4>
  <form action="cutter_purchase_list.php" name="search" method="get">
    <table>
      <tr>
        <th>类型：</th>
...
<div id="table_list">
  ...
  if($result_cutter_list->num_rows){
	  ...
	  $sql_surplus_list = "SELECT `db_cutter_purchase_list`.`cutterid`,SUM(`db_cutter_order_list`.`surplus`) AS `surplus` FROM `db_cutter_order_list` INNER JOIN `db_cutter_purchase_list` ON `db_cutter_purchase_list`.`purchase_listid` = `db_cutter_order_list`.`purchase_listid` WHERE `db_cutter_purchase_list`.`cutterid` IN ($array_cutterid) AND `db_cutter_order_list`.`surplus` > 0 GROUP BY `db_cutter_purchase_list`.`cutterid`";
	  $result_surplus_list = $db->query($sql_surplus_list);
	  if($result_surplus_list->num_rows){
		  while($row_surplus_list = $result_surplus_list->fetch_assoc()){
		  $array_surplus_list[$row_surplus_list['cutterid']] = $row_surplus_list['surplus'];
		  }
	  }else{
		  $array_surplus_list = array();
	  }
  ?>
  <table>
    <tr>
    ...
    while($row_cutter_list = $result_cutter_list->fetch_assoc()){
		...
		$cutter_surplus = array_key_exists($cutterid,$array_surplus_list)?$array_surplus_list[$cutterid]:0;
	?>
    <tr>
