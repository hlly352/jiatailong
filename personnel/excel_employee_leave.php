...
$objActSheet->setTitle('请假单'.date('Y-m-j')."-BY_JTL"); 
...
$objActSheet->setCellValue('B1', '请假单号');
...
$objActSheet->setCellValue('H1', '请假小时(H)');
$objActSheet->setCellValue('I1', '有效小时(H)');
$objActSheet->setCellValue('J1', '类型');
$objActSheet->setCellValue('K1', '班次');
$objActSheet->setCellValue('L1', '状态');
$objActSheet->setCellValue('M1', '确认人');

$sql = $_SESSION['employee_leave_confirm'] . " ORDER BY `db_employee_leave`.`leaveid` DESC";
$result = $db->query($sql);
if($result->num_rows){
	$i = 2;
	while($row = $result->fetch_assoc()){
		...
		$objActSheet->setCellValue('B'.$i, $row['leave_num']);
		...
		$objActSheet->setCellValue('H'.$i, $row['leavetime']);
		$objActSheet->setCellValue('I'.$i, $row['leavetime_valid']);
		$objActSheet->setCellValue('J'.$i, $row['vacation_name']);
		$objActSheet->setCellValue('K'.$i, $array_work_shift[$row['work_shift']]);
		$objActSheet->setCellValue('L'.$i, $array_status[$row['leave_status']]);
		$objActSheet->setCellValue('M'.$i, $confirmer_name);
		$i++;
	}
...
$m_strOutputExcelFileName = '请假单_'.date('Y-m-j_H_i_s').".xls";
//输到文件   
