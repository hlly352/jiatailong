...
if($_GET['submit']){
	...
	$sqlwhere = " WHERE `db_mould`.`mould_number` LIKE '%$mould_number%' AND `db_mould_material`.`material_number` LIKE '%$material_number%' AND `db_mould_material`.`material_name` LIKE '%$material_name%' AND `db_mould_material`.`specification` LIKE '%$specification%'";
}
  ...
  <form action="" name="search" method="get">
    <table>
      <tr>
        ...
        <input type="button" name="button" value="导出" class="button" onclick="location.href='excel_mould_material.php'" /></td>
      </tr>
    </table>
  </form>
</div>
<div id="table_list">
  ...
  if($result->num_rows){
	  while($row_id = $result_id->fetch_assoc()){
		  $array_id .= $row_id['materialid'].',';
	  }
	  $array_id = rtrim($array_id,',');
	  $sql_group = "SELECT `materialid`,`plan_date` FROM `db_material_order_list` WHERE `materialid` IN ($array_id)";
	  $result_group = $db->query($sql_group);
	  if($result_group->num_rows){
		  while($row_group = $result_group->fetch_assoc()){
			  $array_group[$row_group['materialid']] = $row_group['plan_date']; 
		  }
	  }else{
		  $array_group = array();
	  }
  ?>
    <table>
      <tr>
        ...
        <th width="6%">品牌</th>
        <th width="4%">备件数量</th>
        <th width="6%">计划回厂时间</th>
        <th width="12%">备注</th>
      </tr>
      <?php
      while($row = $result->fetch_assoc()){
		  $materialid = $row['materialid'];
		  $plan_date = array_key_exists($materialid,$array_group)?$array_group[$materialid]:'--';
	  ?>
      <tr>
