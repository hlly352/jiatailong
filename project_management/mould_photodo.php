...
if($_POST['submit']){
	...
	if($_FILES['file']['name']){
		...
		if(is_array($array_upfile = $upload->array_upload_file)){
			...
			$sql_image = "SELECT `image_filedir`,`image_filename` FROM `db_mould` WHERE `mouldid` = '$mouldid'";
			...
			$sqlfile = "UPDATE `db_mould` SET `image_filedir` = '$filedir',`image_filename` = '$filename' WHERE `mouldid` = '$mouldid'";
			$db->query($sqlfile);
		}
	}
