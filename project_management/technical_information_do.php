...
<?php
	...
	if(empty($informationid)){
		//判断在db_technical_information 表中是否已经有信息
		...
		//插入除文件的信息
		$sql_list_add = "INSERT INTO `db_technical_information_list`(`informationid`,`ueditor`,`data_name`,`do_employeeid`,`checker`,`manager`,`approvaler`,`geter`,`employeeid`,`dodate`) VALUES('$informationid','$ueditor','$data_name','$do_employeeid','$checker','$manager','$approvaler','$geter','$employeeid','$date')";
		$db->query($sql_list_add);
		$information_listid  = $db->insert_id;
	}else{
		//更新除文件的信息
		$sql_list_update = "UPDATE `db_technical_information_list` SET `do_employeeid` = '$do_employeeid',`checker` = '$checker',`manager` = '$manager',`approvaler` = '$approvaler',`geter` = '$geter',`employeeid` = '$employeeid',`dodate` = '$date',`ueditor` = '$ueditor' WHERE `information_listid` = '$information_listid'";
		$db->query($sql_list_update);
	}
	//在db_technical_information 表中标记创建的信息
	//把地址填入到对应的表中
	...
	//新建当前项目中没有的模具信息
		...
		$sql_remark = "UPDATE `db_technical_information` SET `{$data_name}` = '$information_listid' WHERE `information_id` = '$informationid'";
	}
	$db->query($sql_remark);
	//限制上传文件的大小
	...
	if($_FILES['file']['name']){
		...
		if($db->affected_rows){
			...
			$sql_file_add = "UPDATE `db_technical_information` SET `{$data_name}_path` = CONCAT_WS(',',`{$data_name}_path`,'$information_fileid') WHERE `information_id` = '$informationid'";
