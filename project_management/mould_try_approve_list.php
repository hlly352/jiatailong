...
$sql = "SELECT `db_mould_try`.`tryid`,`db_mould_try`.`mould_size`,CONCAT('T',`db_mould_try`.`try_times`) AS `try_times`,`db_mould_try`.`tonnage`,`db_mould_try`.`molding_cycle`,`db_mould_try`.`plastic_material_color`,`db_mould_try`.`plastic_material_offer`,`db_mould_try`.`product_weight`,`db_mould_try`.`product_quantity`,`db_mould_try`.`material_weight`,`db_mould_try`.`plan_date`,`db_mould_try`.`approve_status`,`db_mould_try`.`try_status`,`db_mould`.`mould_number`,`db_mould`.`cavity_number`,`db_mould`.`difficulty_degree`,`db_mould`.`plastic_material`,`db_mould`.`assembler`,`db_mould_try_cause`.`try_causename`,`db_employee`.`employee_name` FROM `db_mould_try` INNER JOIN `db_mould` ON `db_mould`.`mouldid` = `db_mould_try`.`mouldid` INNER JOIN `db_employee` ON `db_employee`.`employeeid` = `db_mould_try`.`employeeid` INNER JOIN `db_mould_try_cause` ON `db_mould_try_cause`.`try_causeid` = `db_mould_try`.`try_causeid` WHERE `db_mould_try`.`approve_status` = 'A' AND `db_mould_try`.`try_status` = 1 AND `db_mould_try`.`approver` = '$employeeid' $sqlwhere";
$result = $db->query($sql);
...
<div id="table_list">
  ...
  <table>
    ...
    <tr>
      ...
      <td><a href="mould_try_approve.php?id=<?php echo $tryid; ?>"><img src="../images/system_ico/approve_10_10.png" width="10" height="10" /></a></td>
    </tr>
    <?php } ?>
  </table>
  <div id="page">
