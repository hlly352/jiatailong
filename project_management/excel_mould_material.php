...
$objActSheet->setTitle('模具'.$mould_number.'物料清单'.date('Y-m-j')."-BY_JTL"); 
...
if($result->num_rows){
	...
	while($row = $result->fetch_assoc()){
		...
		$objActSheet->setCellValue('C'.$i, $row['material_date']);
		$objActSheet->setCellValue('D'.$i, $row['material_list_number']);
		$objExcel->getActiveSheet()->setCellValueExplicit('E'.$i,$row['material_list_sn'],PHPExcel_Cell_DataType::TYPE_STRING);
		$objActSheet->setCellValue('F'.$i, $row['material_number']);
		$objActSheet->setCellValue('G'.$i, $row['material_name']);
		$objActSheet->setCellValue('H'.$i, $row['specification']);
		$objActSheet->setCellValue('I'.$i, $row['material_quantity']);
		$objActSheet->setCellValue('J'.$i, $row['texture']);
		$objActSheet->setCellValue('K'.$i, $row['hardness']);
		$objActSheet->setCellValue('L'.$i, $row['brand']);
		$objActSheet->setCellValue('M'.$i, $row['spare_quantity']);
		$objActSheet->setCellValue('N'.$i, $row['remark']);
		$i++;
	}

//格式
$objActSheet->getStyle('A1:N'.($i-1))->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN); //设置单元格为实线
$objActSheet->getStyle('A1:N'.($i-1))->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);  //水平居中
$objActSheet->getStyle('A1:N'.($i-1))->getAlignment()->setVertical(PHPExcel_Style_Alignment::VERTICAL_CENTER);// 竖直居中
...
$m_strOutputExcelFileName = '模具'.$mould_number.'物料清单_'.date('Y-m-j_H_i_s').".xls";
//输到文件   
