...
$sql = "SELECT * FROM `db_material_funds_plan` WHERE (`db_material_order`.`order_date` BETWEEN '$sdate' AND '$edate') $sqlwhere";
$result = $db->query($sql);
    ...
    <table>
      <tr>
        ...
        <td><input type="submit" name="submit" value="查询" class="button" />
          <input type="button" name="button" value="添加" class="button" onclick="location.href='funds_plan.php?action=add'" /></td>
      </tr>
    </table>
  </form>
</div>
<div id="table_list">
  ...
  if($result->num_rows){
	  ...
	  //订单是否有出入库记录
	  $sql_material_inout = "SELECT `db_material_order_list`.`orderid` FROM `db_material_funds_plan` INNER JOIN `db_material_order_list` ON `db_material_order_list`.`listid` = `db_material_inout`.`listid` GROUP BY `db_material_order_list`.`orderid`";
