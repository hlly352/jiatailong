  ...
  <form action="" name="search" method="get">
    <table>
      ...
      ?>
        ...
        <td><input type="submit" name="submit" value="查询" class="button" />
          <input type="button" name="button" value="导出" class="button" onclick="location.href='excel_cutter_inquiry.php'" /></td>
      </tr>
    </table>
  </form>
</div>
<div id="table_list">
  <?php if($result->num_rows){ ?>
  <form action="cutter_inquiry_listdo.php" name="list" method="post">
    <table>
      <tr>
        <th width="4%">ID</th>
        <th width="8%">申购单号</th>
      ...
      <tr>
        <td><input type="checkbox" name="id[]" value="<?php echo $row['inquiryid']; ?>" checked="checked" /></td>
...
$sql = "SELECT * FROM `db_mould_other_material` WHERE `status`='D' $sqlwhere";
$result = $db->query($sql);
