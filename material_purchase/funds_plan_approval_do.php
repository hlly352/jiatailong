...
$employeeid = $_SESSION['employee_info']['employeeid'];
	...
	if($action == 'approval'){
		...
		//把对应的项目添加到付款列表中
		$plan_list_sql = "SELECT `db_funds_plan_list`.`accountid`,`db_funds_plan_list`.`plan_amount`,`db_material_account`.`tot_amount`,`db_material_account`.`supplierid`,`db_material_invoice_list`.`invoice_no` FROM `db_funds_plan_list` INNER JOIN `db_material_account` ON `db_funds_plan_list`.`accountid` = `db_material_account`.`accountid` INNER JOIN `db_material_invoice_list` ON `db_material_invoice_list`.`accountid` = `db_material_account`.`accountid` WHERE `db_funds_plan_list`.`planid` = '$planid'";
