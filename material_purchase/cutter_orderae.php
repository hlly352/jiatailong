...
<div id="table_sheet">
  ...
  <?php
	  ...
	  $sql = "SELECT `order_number`,`order_date`,`delivery_cycle`,`supplierid`,`order_status` FROM `db_cutter_order` WHERE `orderid` = '$orderid' AND `employeeid` = '$employeeid' AND `orderid` NOT IN (SELECT `db_cutter_order_list`.`orderid` FROM `db_cutter_inout` INNER JOIN `db_cutter_order_list` ON `db_cutter_order_list`.`listid` = `db_cutter_inout`.`listid` GROUP BY `db_cutter_order_list`.`orderid`)";
	  $result = $db->query($sql);
	  if($result->num_rows){
