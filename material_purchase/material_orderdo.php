...
if($_POST['submit']){
	...
	if($action == "add"){
		...
		if($pay_type == 'P'){
			...
			if($order_status == 1){
				//查询当前订单金额
				$order_amount_sql = "SELECT `db_material_order`.`supplierid`,SUM(`db_material_order_list`.`actual_quantity` * `db_material_order_list`.`unit_price`) AS `order_amount` FROM `db_material_order`,`db_material_order_list`.`process_cost` INNER JOIN `db_material_order_list` ON `db_material_order`.`orderid` = `db_material_order_list`.`orderid` WHERE `db_material_order`.`orderid` = '$orderid' ";
				$result_order_amount = $db->query($order_amount_sql);
				if($result_order_amount){<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
<?php
require_once '../global_mysql_connect.php';
require_once '../function/function.php';
require_once 'shell.php';
		...
		if($pay_type == 'P'){
			...
			if($order_status == 1){
				...
				if($result_order_amount){
					...
					$time = date('Y-m-d');
					//把订单信息插入到对账汇总表中
					$account_sql = "INSERT INTO `db_material_account`(`account_time`,`tot_amount`,`supplierid`,`employeeid`,`orderidlist`,`account_type`,`status`,`tot_process_cost`) VALUES(DATE_FORMAT(NOW(),'%Y-%m-%d'),'$order_amount','$supplierid','$employeeid','$orderid','M','Y','$process_cost')";
