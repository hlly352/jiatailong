...
if($system_info[0] == 1){
  ...
  GROUP BY `mold_id`)".$sqlwhere."AND `is_approval` = '1' AND `is_deal` = '1'";
  } else {
  $sql = "SELECT * FROM `db_mould_data` 
  WHERE time in (
  SELECT max(a.`time`)
...
function getdate(timestamp) {
  ...
  $(function(){
    ...
    $('.show_list').live('click',function(){
      ...
      $('.show').each(function(){
        window.open('mould_show.php?action=show&id='+mold_dataid,'_self');
      })
    })
  //提交批量导出
    ...
    ?>
     <tr class="show">
       <td><input type="checkbox" name="id[]" value="<?php echo $mould_dataid; ?>"<?php if($count > 0) echo " disabled=\"disabled\""; ?> /></td>
        ...
        <td class="show_list">&yen;<?php echo $row['deal_price']?></td>
        <td class="show_list">&yen;<?php echo $row['indoor_price']?></td>
