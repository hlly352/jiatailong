...
<div id="div_tb">
  ...
  <?php
  $type_key = 'A';
  $sql = "SELECT `db_mould_quote_list`.`specification`,`db_mould_quote_list`.`number`,`db_mould_quote_list`.`length`,`db_mould_quote_list`.`width`,`db_mould_quote_list`.`height`,`db_mould_quote_list`.`weight`,`db_mould_quote_list`.`unit_price`,`db_mould_quote_list`.`total_price`,`db_quote_item`.`item_name` FROM `db_mould_quote_list` INNER JOIN `db_quote_item` ON `db_quote_item`.`itemid` = `db_mould_quote_list`.`itemid` INNER JOIN `db_quote_item_type` ON `db_quote_item_type`.`item_typeid` = `db_quote_item`.`item_typeid` WHERE `db_quote_item_type`.`item_type_sn` = '$type_key' AND `db_mould_quote_list`.`quoteid` = '$quoteid' ORDER BY `db_quote_item`.`item_sn` ASC";
  $result = $db->query($sql);
  ...
  <table>
    ...
    <tr>
      ...
      <td width="6%"><?php echo $row['length']; ?></td>
      <td width="3%" style="text-align:center;">*</td>
      <td width="6%"><?php echo $row['width']; ?></td>
      <td width="3%" style="text-align:center;">*</td>
      <td width="6%"><?php echo $row['height']; ?></td>
      <td><?php echo $row['weight']; ?></td>
      <td><?php echo $row['unit_price']; ?></td>
      <td><?php echo number_format($row['total_price'],2); ?></td>
      <?php if($i == 1){ ?>
      <td rowspan="<?php echo $count; ?>"><?php echo number_format($sum_price,2); ?></td>
      <?php } ?>
    </tr>
    <?php
    $i++;
  ...
  <?php
  $type_key = 'B';
  $sql = "SELECT `db_mould_quote_list`.`weight`,`db_mould_quote_list`.`unit_price`,`db_mould_quote_list`.`total_price`,`db_quote_item`.`item_name` FROM `db_mould_quote_list` INNER JOIN `db_quote_item` ON `db_quote_item`.`itemid` = `db_mould_quote_list`.`itemid` INNER JOIN `db_quote_item_type` ON `db_quote_item_type`.`item_typeid` = `db_quote_item`.`item_typeid` WHERE `db_quote_item_type`.`item_type_sn` = '$type_key' AND `db_mould_quote_list`.`quoteid` = '$quoteid' ORDER BY `db_quote_item`.`item_sn` ASC";
  $result = $db->query($sql);
  if($count = $result->num_rows){
	  $item_typename = array_key_exists($type_key,$array_group)?$array_group[$type_key]['item_typename']:'--';
	  $sum_price = array_key_exists($type_key,$array_group)?$array_group[$type_key]['sum_price']:0;
  ...
  <?php
  $type_key = 'C';
  $sql = "SELECT `db_mould_quote_list`.`specification`,`db_mould_quote_list`.`supplier`,`db_mould_quote_list`.`number`,`db_mould_quote_list`.`unit_price`,`db_mould_quote_list`.`total_price`,`db_quote_item`.`item_name` FROM `db_mould_quote_list` INNER JOIN `db_quote_item` ON `db_quote_item`.`itemid` = `db_mould_quote_list`.`itemid` INNER JOIN `db_quote_item_type` ON `db_quote_item_type`.`item_typeid` = `db_quote_item`.`item_typeid` WHERE `db_quote_item_type`.`item_type_sn` = '$type_key' AND `db_mould_quote_list`.`quoteid` = '$quoteid' ORDER BY `db_quote_item`.`item_sn` ASC";
  $result = $db->query($sql);
  if($count = $result->num_rows){
	  $item_typename = array_key_exists($type_key,$array_group)?$array_group[$type_key]['item_typename']:'--';
	  $sum_price = array_key_exists($type_key,$array_group)?$array_group[$type_key]['sum_price']:0;
  ...
  <?php
  $type_key = 'D';
  $sql = "SELECT `db_mould_quote_list`.`hour`,`db_mould_quote_list`.`unit_price`,`db_mould_quote_list`.`total_price`,`db_quote_item`.`item_name` FROM `db_mould_quote_list` INNER JOIN `db_quote_item` ON `db_quote_item`.`itemid` = `db_mould_quote_list`.`itemid` INNER JOIN `db_quote_item_type` ON `db_quote_item_type`.`item_typeid` = `db_quote_item`.`item_typeid` WHERE `db_quote_item_type`.`item_type_sn` = '$type_key' AND `db_mould_quote_list`.`quoteid` = '$quoteid' ORDER BY `db_quote_item`.`item_sn` ASC";
  $result = $db->query($sql);
  if($count = $result->num_rows){
	  $item_typename = array_key_exists($type_key,$array_group)?$array_group[$type_key]['item_typename']:'--';
	  $sum_price = array_key_exists($type_key,$array_group)?$array_group[$type_key]['sum_price']:0;
  ...
  <?php
  $type_key = 'F';
  $sql = "SELECT `db_mould_quote_list`.`descripition`,`db_mould_quote_list`.`total_price`,`db_quote_item`.`item_name`,`db_quote_item`.`item_sn` FROM `db_mould_quote_list` INNER JOIN `db_quote_item` ON `db_quote_item`.`itemid` = `db_mould_quote_list`.`itemid` INNER JOIN `db_quote_item_type` ON `db_quote_item_type`.`item_typeid` = `db_quote_item`.`item_typeid` WHERE `db_quote_item_type`.`item_type_sn` = '$type_key' AND `db_mould_quote_list`.`quoteid` = '$quoteid' ORDER BY `db_quote_item`.`item_sn` ASC";
  $result = $db->query($sql);
  if($count = $result->num_rows){
	  $item_typename = array_key_exists($type_key,$array_group)?$array_group[$type_key]['item_typename']:'--';
	  $sum_price = array_key_exists($type_key,$array_group)?$array_group[$type_key]['sum_price']:0;
