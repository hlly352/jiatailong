...
$(function(){
  ...
  $(".heat_weight,.heat_unit_price").live("blur",function(){
    ...
    $(this).parent().parent().children().eq(3).children().val(heats_totals);
  ...
  $(".design_hour,.design_unit_price").live("blur",function(){
    if($(this).attr('class') == 'design_hour'){
      
      var designs_totals = ($(this).val())*( $(this).parent().next().children().val());
    }else{
      var designs_totals = ($(this).val())*($(this).parent().prev().children().val());
    }
    $(this).parent().parent().children().eq(3).children().val(designs_totals);
     sum_tds(".design_trs",1,2,"#total_designs");
     //计算其它费用及模具价格
        sum_other_fee();
  ...
  $(".standard_number,.standard_unit_price").live("blur",function(){
    ...
    $(this).parent().parent().children().eq(5).children().val(standard_totals);
  ...
  $(".cavity_style_length").live('change',function(){
    ...
    var p_length = $('.material_length').eq(cavity_nu*6+1).val();
    var p_width = $('.material_width').eq(cavity_nu*6+1).val();
    //删除宽的布局
    $(".first_style_width").eq(cavity_nu).siblings().remove();
    $(".first_width").eq(cavity_nu).siblings().remove();
    //获取当前布局的值
    var cavity_style_length = parseInt($(this).val());
    //计算型腔布局的宽并加入到布局选项中
    ...
    var p_len = $('.p_length').eq(cavity_nu).val();
    var p_wid = $('.p_width').eq(cavity_nu).val();
    //添加布局宽度
  ...
  $('#currency').live('change',function(){
  	...
  	if(current_currency == 'rmb' || current_currency == 'rmb_vat'){
  		$('#mold_rate').val('1');
  	} else {
  		$('#mold_rate').val('');
 	} 
  ...
  $('#mold_agreement_price,#mold_rate,#currency,#mold_rate').live('change',function(){
    ...
    if(mold_agreement_price && mold_rate){
      ...
      if(currency =='rmb_vat'){
        ...
        //计算税收
        var order_vat = parseFloat(mold_agreement_price * mold_rate  /1.13 * 0.13);
        ...
        //计算税收
	        if(currency == 'rmb'){
	        	var order_vat = mold_agreement_price * mold_rate * 0.13;
	        	order_vat = order_vat.toFixed(2);
	        } else {
	          var order_vat = 0;
	        }
  ...
  $('#mould_deal').click(function(){
    ...
    var mold_id = $(this).prev().val();
    window.open('mould_datado.php?action=mould_deal&unit_price='+mold_agreement_price+'&deal_price='+mold_deal_price+'&agreement_price='+mold_agreement_price+'&mold_rate='+mold_rate+'&currency='+currency+'&indoor_price='+mold_indoor_price+'&id='+mold_id+'&order_vat='+order_vat,'_self');
  })
  //未审批时修改
 ...
 <?php if(!$_GET['version']){ ?>
  <script type="text/javascript" charset="utf-8">
  	$(document).ready(function(){
  		$('.cavity_length,.cavity_width,.cavity_style_length,.cavity_style_width').prop('disabled',true);
  		$('input').not('#mold_agreement_price,#mold_indoor_price,#mold_deal_price,#mold_rate,#order_vat').prop('disabled',true);
      $('.adder').unbind('click');
           ...
           <?php }else { ?>
              <tr>
                ...
                <td colspan="5">
                 ...
                 <td>汇率</td>
                 <td colspan="5">
                  <input type="text" name="" id="mold_rate" value="1" placeholder="">
                 </td>
