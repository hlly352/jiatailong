...
$objActSheet->setTitle('刀具申领明细'.date('Y-m-j')."-BY_JTL"); 
...
$objActSheet->setCellValue('G1', '申领数量');
$objActSheet->setCellValue('H1', '入库数量');
$objActSheet->setCellValue('I1', '单位');
$objActSheet->setCellValue('J1', '模具编号');
$objActSheet->setCellValue('K1', '申购人');
$objActSheet->setCellValue('L1', '申购日期');
$objActSheet->setCellValue('M1', '计划回厂日期');

$sql = $_SESSION['apply_list']." ORDER BY `db_cutter_apply`.`applyid` DESC,`db_cutter_apply_list`.`apply_listid` DESC";
$result = $db->query($sql);
if($result->num_rows){
	$i = 2;
	while($row = $result->fetch_assoc()){
		$objActSheet->setCellValue('A'.$i, $i-1);
		$objActSheet->setCellValue('B'.$i, $row['apply_number']);
		...
		$objActSheet->setCellValue('H'.$i, $row['out_quantity']);
		$objActSheet->setCellValue('I'.$i, '件');
		$objActSheet->setCellValue('J'.$i, $row['mould_number']);
		$objActSheet->setCellValue('K'.$i, $row['employee_name']);
		$objActSheet->setCellValue('L'.$i, $row['apply_date']);
...
$m_strOutputExcelFileName = '刀具申领明细_'.date('Y-m-j_H_i_s').".xls";
//输到文件   
