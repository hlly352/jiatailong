...
$objActSheet->setTitle('物料订单付款记录'.date('Y-m-j')."-BY_JTL"); 
...
$objActSheet->setCellValue('H1', '单价(含税)');
$objActSheet->setCellValue('I1', '税率');
$objActSheet->setCellValue('J1', '金额(含税)');
$objActSheet->setCellValue('K1', '加工费');
...
$sql = $_SESSION['material_order_payment_list']." ORDER BY `db_cash_pay`.`pay_date` DESC,`db_cash_pay`.`payid` DESC";
$result = $db->query($sql);
if($result->num_rows){
	$i = 2;
	while($row = $result->fetch_assoc()){
		...
		$objActSheet->setCellValue('F'.$i, $row['actual_quantity']);
		$objActSheet->setCellValue('G'.$i, $row['unit_name']);
		$objActSheet->setCellValue('H'.$i, $row['unit_price']);
		$objActSheet->setCellValue('I'.$i, ($row['tax_rate']*100).'%');
		$objActSheet->setCellValue('J'.$i, $row['amount']);
		$objActSheet->setCellValue('K'.$i, $row['process_cost']);
		$objActSheet->setCellValue('L'.$i, $row['total_amount']);
...
$m_strOutputExcelFileName = '物料订单付款记录_'.date('Y-m-j_H_i_s').".xls";
//输到文件   
