...
$sql = "SELECT `db_material_account`.`accountid`,`db_material_account`.`account_time`,`db_material_account`.`tot_cut_payment`,`db_material_account`.`tot_process_cost`,`db_material_account`.`tot_prepayment`,`db_material_account`.`tot_cancel_amount`,`db_supplier`.`supplier_cname`,`db_material_account`.`tot_amount`,(`db_material_account`.`tot_amount` + `db_material_account`.`tot_process_cost` - `db_material_account`.`tot_cut_payment`  - `db_material_account`.`tot_cancel_amount` - `db_material_account`.`tot_prepayment`) AS `amount`  FROM `db_material_account` INNER JOIN `db_supplier` ON `db_material_account`.`supplierid` = `db_supplier`.`supplierid` INNER JOIN `db_material_account_list` ON `db_material_account`.`accountid` = `db_material_account_list`.`accountid` INNER JOIN `db_material_inout` ON `db_material_account_list`.`inoutid` = `db_material_inout`.`inoutid` WHERE `db_material_inout`.`account_status` = 'F' AND (`db_material_account`.`account_time` BETWEEN '$sdate' AND '$edate')".$sqlwhere."GROUP BY `db_material_account`.`accountid`";
