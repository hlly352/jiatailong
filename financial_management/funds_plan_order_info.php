...
?>
  ...
  <?php }elseif($action == 'pay'){
        $accountid = $_GET['accountid'];
        //通过对账单号在对账详情表中查找订单信息
         $order_sql = "SELECT `db_funds_plan_list`.`tot_payment`,`db_funds_plan_list`.`listid`,`db_account_order_list`.`order_number`,`db_account_order_list`.`order_amount`,`db_account_order_list`.`process_cost`,`db_account_order_list`.`cancel_amount`,`db_account_order_list`.`cut_payment`,(`db_account_order_list`.`order_amount` + `db_account_order_list`.`process_cost` - `db_account_order_list`.`cancel_amount` - `db_account_order_list`.`cut_payment`) AS `total_amount`,`db_funds_plan_list`.`plan_amount` FROM `db_funds_plan_list` INNER JOIN `db_account_order_list` ON `db_funds_plan_list`.`order_listid` = `db_account_order_list`.`listid` WHERE `db_funds_plan_list`.`planid` = '$planid' AND `db_account_order_list`.`accountid` = '$accountid' AND `db_funds_plan_list`.`plan_amount` > `db_funds_plan_list`.`tot_payment`";
  ...
  <?php }elseif($action == 'over'){  
      $accountid = $_GET['accountid'];
        //通过对账单号在对账详情表中查找订单信息
         $order_sql = "SELECT `db_funds_plan_list`.`planid`,`db_funds_plan_list`.`tot_payment`,`db_funds_plan_list`.`listid`,`db_account_order_list`.`order_number`,`db_account_order_list`.`order_amount`,`db_account_order_list`.`process_cost`,`db_account_order_list`.`cancel_amount`,`db_account_order_list`.`cut_payment`,`db_funds_plan_list`.`plan_amount`,`db_funds_plan_payment`.`payment`,(`db_funds_plan_payment`.`listid`) AS `payment_listid` FROM `db_funds_plan_list` INNER JOIN `db_account_order_list` ON `db_funds_plan_list`.`order_listid` = `db_account_order_list`.`listid`  INNER JOIN `db_funds_plan_payment` ON `db_funds_plan_list`.`listid` = `db_funds_plan_payment`.`plan_listid` WHERE `db_funds_plan_list`.`planid` = '$planid' AND `db_account_order_list`.`accountid` = '$accountid' AND `db_funds_plan_payment`.`pay_status` = 'A'";
...
<?php }elseif($action == 'financial'){ 
      ...
      <table>
          <tr>
            <td colspan="15">
              <input type="button" name="" value="审核" class="button" onclick="window.location.href='funds_apply_do.php?from=financial&action=complete&planid=<?php echo $planid ?>&accountid=<?php echo $accountid ?>'" />
              <input type="button" name="" value="撤回" class="button"  onclick="window.location.href='funds_apply_do.php?from=financial&action=back&planid=<?php echo $planid ?>&accountid=<?php echo $accountid ?>'" />
              <input type="button" name="button" value="返回" class="button" onclick="window.history.go(-1)" />
            </td>
          </tr>
      </table>
