...
$objActSheet->setTitle('刀具订单明细'.date('Y-m-j')."-BY_JTL"); 
...
$objActSheet->setCellValue('N1', '现金');
$objActSheet->setCellValue('O1', '供应商');
$objActSheet->setCellValue('P1', '订单日期');
$objActSheet->setCellValue('Q1', '计划回厂时间');

$sql = $_SESSION['cutter_orderlist']." ORDER BY `db_cutter_order`.`orderid` DESC,`db_cutter_order_list`.`listid` DESC";
$result = $db->query($sql);
...
if($result->num_rows){
	...
	while($row = $result->fetch_assoc()){
		...
		$objActSheet->setCellValue('I'.$i, $row['in_quantity']);
		...
		$objActSheet->setCellValue('N'.$i, $array_is_status[$row['iscash']]);
		$objActSheet->setCellValue('O'.$i, $row['supplier_cname']);
		$objActSheet->setCellValue('P'.$i, $row['order_date']);
		$objActSheet->setCellValue('Q'.$i, $row['plan_date']);
		$i++;
	}
...
$m_strOutputExcelFileName = '刀具订单明细_'.date('Y-m-j_H_i_s').".xls";
//输到文件   
