...
$objActSheet->setTitle('物料订单记录'.date('Y-m-j')."-BY_JTL"); 
...
$objActSheet->setCellValue('L1', '税率');
$objActSheet->setCellValue('M1', '金额(含税)');
$objActSheet->setCellValue('N1', '加工费');
$objActSheet->setCellValue('O1', '现金');
$objActSheet->setCellValue('P1', '供应商');
$objActSheet->setCellValue('Q1', '订单日期');
$objActSheet->setCellValue('R1', '计划回厂时间');

$sql = $_SESSION['order_list']." ORDER BY `db_material_order`.`order_date` DESC,`db_material_order_list`.`listid` DESC";
$result = $db->query($sql);
...
if($result->num_rows){
	...
	$sql_pay_amount = "SELECT `linkid`,SUM(`pay_amount`) AS `total_pay_amount` FROM `db_cash_pay` WHERE `data_type` = 'M' AND `linkid` IN ($array_id) GROUP BY `linkid`";
	...
	while($row = $result->fetch_assoc()){
		...
		$objActSheet->setCellValue('L'.$i, ($row['tax_rate']*100).'%');
		$objActSheet->setCellValue('M'.$i, $row['amount']);
		$objActSheet->setCellValue('N'.$i, $row['process_cost']);
		$objActSheet->setCellValue('O'.$i, $pay_amount);
		$objActSheet->setCellValue('P'.$i, $row['supplier_cname']);
		$objActSheet->setCellValue('Q'.$i, $row['order_date']);
		$objActSheet->setCellValue('R'.$i, $row['plan_date']);
		$i++;
	}
...
$m_strOutputExcelFileName = '物料订单记录_'.date('Y-m-j_H_i_s').".xls";
//输到文件   
