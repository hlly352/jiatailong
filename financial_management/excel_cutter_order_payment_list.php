...
$objActSheet->setTitle('刀具订单付款记录'.date('Y-m-j')."-BY_JTL"); 
...
$objActSheet->setCellValue('M1', '付款现金');
$objActSheet->setCellValue('N1', '付款日期');
$objActSheet->setCellValue('O1', '付款人');
$objActSheet->setCellValue('P1', '供应商');
$objActSheet->setCellValue('Q1', '订单日期');

$sql = $_SESSION['cutter_order_payment_list']." ORDER BY `db_cash_pay`.`pay_date` DESC,`db_cash_pay`.`payid` DESC";
$result = $db->query($sql);
if($result->num_rows){
	$i = 2;
	while($row = $result->fetch_assoc()){
		...
		$objActSheet->setCellValue('M'.$i, $row['pay_amount']);
		$objActSheet->setCellValue('N'.$i, $row['pay_date']);
		$objActSheet->setCellValue('O'.$i, $row['employee_name']);
...
$m_strOutputExcelFileName = '刀具订单付款记录_'.date('Y-m-j_H_i_s').".xls";
//输到文件   
