...
if($_GET['submit']){
	...
	$sql = "SELECT `apply_date`,SUM((`odometer_finish`-`odometer_start`)*`charge`+(ROUND(`wait_time`)*`charge_wait`)+`charge_toll`+`charge_parking`) AS `cost` FROM `db_vehicle_list` WHERE `vehicle_status` = 1 AND `approve_status` = 'B' AND `reckoner` != 0 AND DATE_FORMAT(`apply_date`,'%Y-%m') = '$month' GROUP BY `apply_date`";
	$result = $db->query($sql);
	if($result->num_rows){
		while($row = $result->fetch_assoc()){
			$array_data[$row['apply_date']] = $row['cost'];
		}
	}else{
		$array_data = array();
	}
	for($i=1;$i<=$month_days;$i++){
		...
		$cost = array_key_exists($date,$array_data)?$array_data[$date]:'0';
		$array_cost[] = $cost;
	}
	//图示处理
	$ydata1 = $array_cost;
	$titles = $month."用车费用日报表";
	//全局变量
