...
$objActSheet->setTitle('快递结算清单'.date('Y-m-j')."-BY_JTL"); 
...
$objActSheet->setCellValue('F1', '快递公司');
$objActSheet->setCellValue('G1', '收件人公司');
$objActSheet->setCellValue('H1', '快递物品');
$objActSheet->setCellValue('I1', '付款方式');
$objActSheet->setCellValue('J1', '结算人');
$objActSheet->setCellValue('K1', '费用');
$objActSheet->setCellValue('L1', '审批状态');
$objActSheet->setCellValue('M1', '状态');
$sql = $_SESSION['employee_express']." ORDER BY `db_employee_express`.`expressid` DESC";
$result = $db->query($sql);
if($result->num_rows){
	$i = 2;
	while($row = $result->fetch_assoc()){
		...
		$objActSheet->setCellValue('B'.$i, $row['express_num']);
		...
		$objActSheet->setCellValue('F'.$i, $row['inc_cname']);
		$objActSheet->setCellValue('G'.$i, $row['consignee_inc']);
		$objActSheet->setCellValue('H'.$i, $row['express_item']);
		$objActSheet->setCellValue('I'.$i, $array_express_paytype[$row['paytype']]);
		$objActSheet->setCellValue('J'.$i, $reckoner_name);
		$objActSheet->setCellValue('K'.$i, $cost);
		$objActSheet->setCellValue('L'.$i, $array_office_approve_status[$row['approve_status']]);
		$objActSheet->setCellValue('M'.$i, $array_status[$row['express_status']]);
		$i++;
	}
...
//设置字体   
$objStyle1 = $objActSheet->getStyle('A2:M'.($i-1)); 
...
$m_strOutputExcelFileName = '快递结算清单_'.date('Y-m-j_H_i_s').".xls";
//输到文件   
