...
$emloyeeid = $_SESSION['employee_info']['employeeid'];
$gooutid = fun_check_int($_GET['id']);
$sql = "SELECT `db_employee_goout`.`gooutid`,`db_employee_goout`.`goout_num`,`db_employee_goout`.`applyer`,`db_employee_goout`.`agenter`,`db_employee_goout`.`apply_date`,`db_employee_goout`.`destination`,`db_employee_goout`.`start_time`,`db_employee_goout`.`finish_time`,`db_employee_goout`.`goout_cause`,`db_employee_goout`.`dotime`,`db_employee_goout`.`approve_status`,`db_applyer`.`employee_name` AS `applyer_name`,`db_agenter`.`employee_name` AS `agenter_name`,ROUND(TIMESTAMPDIFF(MINUTE,`db_employee_goout`.`start_time`,`db_employee_goout`.`finish_time`)/60,1) AS `diffhour` FROM `db_employee_goout` INNER JOIN `db_employee` AS `db_applyer` ON `db_applyer`.`employeeid` = `db_employee_goout`.`applyer` INNER JOIN `db_employee` AS `db_agenter` ON `db_agenter`.`employeeid` = `db_employee_goout`.`agenter` WHERE `db_employee_goout`.`approve_status` = 'A' AND `db_employee_goout`.`goout_status` = 1 AND (`db_applyer`.`superior` = '$emloyeeid' OR (`db_applyer`.`position_type` IN ('A','B') AND `db_employee_goout`.`applyer` = '$emloyeeid')) AND `db_employee_goout`.`gooutid` = '$gooutid'";
$result = $db->query($sql);
?>
...
<div id="table_sheet">
  <h4>出门证审批</h4>
  <form action="employee_goout_approvedo.php" name="employee_goout_approve" method="post">
    <table>
      <tr>
        <th width="20%">出门证号：</th>
