...
if($_GET['submit']){
	...
	if($approve_status){
		$sql_approve_status = " AND `db_employee_overtime`.`approve_status` = '$approve_status'";
	}
	...
	$sqlwhere = " AND `db_employee_overtime`.`overtime_num` LIKE '%$overtime_num%' $sqlapprove_status $sqlovertime_status";
}else{
	$overtime_status = 1;
	$sqlwhere = " AND `db_employee_overtime`.`overtime_status` = $overtime_status";
}
$sql = "SELECT `db_employee_overtime`.`overtimeid`,`db_employee_overtime`.`overtime_num`,`db_employee_overtime`.`confirmer`,`db_employee_overtime`.`apply_date`,`db_employee_overtime`.`start_time`,`db_employee_overtime`.`finish_time`,`db_employee_overtime`.`overtime`,`db_employee_overtime`.`approve_status`,`db_employee_overtime`.`overtime_status`,`db_applyer`.`employee_name` AS `applyer_name`,`db_confirmer`.`employee_name` AS `confirmer_name` FROM `db_employee_overtime` INNER JOIN `db_employee` AS `db_applyer` ON `db_applyer`.`employeeid` = `db_employee_overtime`.`applyer` LEFT JOIN `db_employee` AS `db_confirmer` ON `db_confirmer`.`employeeid` = `db_employee_overtime`.`confirmer` WHERE (`db_employee_overtime`.`apply_date` BETWEEN '$sdate' AND '$edate') AND (`db_employee_overtime`.`applyer` = '$employeeid' OR `db_employee_overtime`.`agenter` = '$employeeid') $sqlwhere";
$result = $db->query($sql);
    ...
    <table>
      <tr>
        ...
        <td><select name="overtime_status">
            ...
            <option value="<?php echo $status_key; ?>"<?php if($status_key == $overtime_status && $overtime_status != NULL) echo " selected=\"selected\""; ?>><?php echo $status_value; ?></option>
            <?php } ?>
          </select></td>
        <td><input type="submit" name="submit" value="查询" class="button" />
          <input type="button" name="button" value="申请" class="button" onclick="location.href='employee_overtimeae.php?action=add'" /></td>
      </tr>
    </table>
  </form>
</div>
<div id="table_list">
  ...
  <form action="employee_overtimedo.php" name="list" method="post">
    <table>
      <tr>
        <th width="4%">ID</th>
        <th width="12%">加班单号</th>
	  ...
	  ?>
      <tr>
        <td><input type="checkbox" name="id[]" value="<?php echo $overtimeid; ?>"<?php if($approve_status != "C") echo " disabled=\"disabled\""; ?> /></td>
        ...
        <td><?php if($approve_status == "C" && $overtime_status == 1){ ?>
          <a href="employee_overtimeae.php?id=<?php echo $overtimeid; ?>&action=edit"><img src="../images/system_ico/edit_10_10.png" width="10" height="10" /></a>
          <?php } ?></td>
