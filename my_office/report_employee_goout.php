...
//查询月度数据
$sql_month = "SELECT `applyer`,ROUND(SUM(TIMESTAMPDIFF(MINUTE,`start_time`,`finish_time`)/60),1) AS `total_time`,COUNT(*) AS `total_count` FROM `db_employee_goout` WHERE DATE_FORMAT(`start_time`,'%Y-%m') = '$month' AND `approve_status` = 'B' AND `goout_status` = 1 GROUP BY `applyer` ORDER BY SUM(TIMESTAMPDIFF(MINUTE,`start_time`,`finish_time`)) ASC";
...
if($_GET['submit']){
	...
	if($orderby == 'B'){
		...
		$sqlorderby = " ORDER BY field(`db_employee_goout`.`applyer`,$applyerid) DESC";
	}elseif($orderby == 'A'){
		$sqlorderby = " ORDER BY SUM(TIMESTAMPDIFF(MINUTE,`db_employee_goout`.`start_time`,`db_employee_goout`.`finish_time`)) DESC";
	}
	$sqlwhere = " AND `db_employee`.`employee_name` LIKE '%$applyer_name%'";
}else{
	$sqlorderby = " ORDER BY SUM(TIMESTAMPDIFF(MINUTE,`db_employee_goout`.`start_time`,`db_employee_goout`.`finish_time`)) DESC";
}
$sql_year = "SELECT `db_employee_goout`.`applyer`,`db_employee`.`employee_name`,ROUND(SUM(TIMESTAMPDIFF(MINUTE,`db_employee_goout`.`start_time`,`db_employee_goout`.`finish_time`)/60),1) AS `total_time`,COUNT(*) AS `total_count` FROM `db_employee_goout` INNER JOIN `db_employee` ON `db_employee`.`employeeid` = `db_employee_goout`.`applyer` INNER JOIN `db_department` ON `db_department`.`deptid` = `db_employee`.`deptid` WHERE DATE_FORMAT(`db_employee_goout`.`start_time`,'%Y') = '$year' AND `db_employee_goout`.`approve_status` = 'B' AND `db_employee_goout`.`goout_status` = 1 $sqlwhere GROUP BY `db_employee_goout`.`applyer`";
