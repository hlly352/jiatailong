...
$sql = "SELECT DATE_FORMAT(`db_employee_express_receive`.`receipt_date`,'%Y-%m') AS `month`,SUM(`db_employee_express_receive`.`cost`) AS `cost` FROM `db_employee_express_receive` INNER JOIN `db_employee` ON `db_employee`.`employeeid` = `db_employee_express_receive`.`receiver` WHERE DATE_FORMAT(`db_employee_express_receive`.`receipt_date`,'%Y') = '$year' AND `db_employee_express_receive`.`express_status` = 1 GROUP BY DATE_FORMAT(`db_employee_express_receive`.`receipt_date`,'%Y-%m')";
$result = $db->query($sql);
if($result->num_rows){
	...
	for($i=1;$i<=12;$i++){
		...
		$array_date1[] = number_format(array_key_exists($month,$array_express)?$array_express[$month]:0,1);
	}
	//print_r($array_date1);
	//图示处理
	$ydata1 = $array_date1;
	$titles = $year.'年快递(收)费年用报表';
	//全局变量
