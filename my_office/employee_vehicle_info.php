...
if($result->num_rows){
	...
	$confirmer_outname = $arr['confirmer_out']?$arr['confirmer_outname']:'--';
	$confirmer_inname = $arr['confirmer_in']?$arr['confirmer_inname']:'--';
	$reckoner_name = $arr['reckoner']?$arr['reckoner_name']:'--';
	$settle_time = $arr['reckoner']?$arr['settle_time']:'--';
?>
<div id="table_sheet">
  <h4>用车信息</h4>
  <form action="approvedo.php" name="approve_vehicle" method="post">
    <table>
      <tr>
        <th width="10%">派车单号：</th>
        <td width="15%"><?php echo $array['vehicle_num'] ?></td>
        <th width="10%">部门：</th>
        <td width="15%"><?php echo $array['dept_name']; ?></td>
        <th width="10%">申请人：</th>
        <td width="15%"><?php echo $array['applyer_name']; ?></td>
        <th width="10%">申请日期：</th>
