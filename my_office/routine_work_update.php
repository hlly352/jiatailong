...
<div id="table_sheet">
  ...
  if($result->num_rows){
	  ...
	  //工作附件(workid+WK)
	  $sql_file = "SELECT `fileid`,`upfilename` FROM `db_upload_file` WHERE `linkcode` = 'RW' AND `linkid` = '$workid' ORDER BY `fileid` ASC";
...
<div id="table_list">
  ...
  <form action="routine_work_updatedo.php" name="routine_work_update_list" method="post">
    ...
    <div id="checkall">
      ...
      <input type="button" name="button" value="添加" class="select_button" onclick="location.href='routine_work_updateae.php?id=<?php echo $workid; ?>&date=<?php echo $date; ?>&action=add'" />
      <input type="hidden" name="action" value="del" />
    </div>
  </form>
  <?php
  }else{
	  echo "<p class=\"tag\">系统提示：暂无例行工作反馈记录，请点击<a href='routine_work_updateae.php?id=".$workid."&date=".$date."&action=add'>添加</a>！</p>";
  }
  ?>
</div>
