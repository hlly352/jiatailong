...
//查询月份数据
$sql_month = "SELECT `applyer`,FORMAT(SUM(`leavetime`),1) AS `total_time`,COUNT(*) AS `total_count` FROM `db_employee_leave` WHERE DATE_FORMAT(`start_time`,'%Y-%m') = '$month' AND `approve_status` = 'B' AND `leave_status` = 1 AND `confirmer` != 0 GROUP BY `applyer` ORDER BY SUM(`leavetime`) ASC";
...
if($_GET['submit']){
	...
	if($orderby == 'B'){
		...
		$sqlorderby = " ORDER BY field(`db_employee_leave`.`applyer`,$applyerid) DESC";
	}elseif($orderby == 'A'){
		$sqlorderby = " ORDER BY SUM(`db_employee_leave`.`leavetime`) DESC";
	}
...
//年度
$sql_year = "SELECT `db_employee_leave`.`applyer`,`db_employee`.`employee_name`,FORMAT(SUM(`db_employee_leave`.`leavetime`),1) AS `total_time`,COUNT(*) AS `total_count` FROM `db_employee_leave` INNER JOIN `db_employee` ON `db_employee`.`employeeid` = `db_employee_leave`.`applyer` INNER JOIN `db_department` ON `db_department`.`deptid` = `db_employee`.`deptid` WHERE DATE_FORMAT(`db_employee_leave`.`start_time`,'%Y') = '$year' AND `db_employee_leave`.`approve_status` = 'B' AND `db_employee_leave`.`leave_status` = 1 AND `db_employee_leave`.`confirmer` != 0 $sqlwhere GROUP BY `db_employee_leave`.`applyer`";
