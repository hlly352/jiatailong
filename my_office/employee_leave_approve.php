...
$sql = "SELECT `db_employee_leave`.`leaveid`,`db_employee_leave`.`leave_num`,`db_employee_leave`.`applyer`,`db_employee_leave`.`agenter`,`db_employee_leave`.`vacationid`,`db_employee_leave`.`apply_date`,`db_employee_leave`.`work_shift`,`db_employee_leave`.`start_time`,`db_employee_leave`.`finish_time`,`db_employee_leave`.`leavetime`,`db_employee_leave`.`leave_cause`,`db_employee_leave`.`dotime`,`db_employee_leave`.`approve_status`,`db_applyer`.`employee_name` AS `applyer_name`,`db_agenter`.`employee_name` AS `agenter_name`,`db_personnel_vacation`.`vacation_name` FROM `db_employee_leave` INNER JOIN `db_employee` AS `db_applyer` ON `db_applyer`.`employeeid` = `db_employee_leave`.`applyer` INNER JOIN `db_employee` AS `db_agenter` ON `db_agenter`.`employeeid` = `db_employee_leave`.`agenter` INNER JOIN `db_personnel_vacation` ON `db_personnel_vacation`.`vacationid` = `db_employee_leave`.`vacationid` WHERE `db_employee_leave`.`approve_status` = 'A' AND `db_employee_leave`.`leave_status` = 1 AND `db_employee_leave`.`approver` = '$employeeid' AND `db_employee_leave`.`leaveid` = '$leaveid'";
$result = $db->query($sql);
?>
...
<div id="table_sheet">
  <h4>请假单审批</h4>
  <form action="employee_leave_approvedo.php" name="employee_leave_approve" method="post">
    <table>
      <tr>
        <th width="20%">请假单号：</th>
      ...
      <tr>
        <th>请假类型：</th>
        <td><?php echo $array['vacation_name']; ?><?php if($vacationid == 2) echo "<font color='red'>(可调休时间为".$overtime_valid."小时)</font>"; ?></td>
      </tr>
      <tr>
        <th>请假事由：</th>
