...
if($_GET['submit']){
	...
	$sqlwhere = " WHERE `db_issuer`.`employee_name` LIKE '%$issuer_name%' AND `db_worker`.`employee_name` LIKE '%$worker_name%' $sql_pdcastatus $sql_pdca_result $sql_workstatus $sql_update_type";
}else{
	$work_status = 1;
	$sqlwhere = " WHERE `db_work`.`work_status` = '$work_status'";
}
$sql = "SELECT `db_work`.`workid`,`db_work`.`work_content`,`db_work`.`issue_date`,`db_work`.`deadline_date`,`db_work`.`finish_date`,`db_work`.`pdca_status`,`db_work`.`work_status`,`db_worker`.`employee_name` AS `worker_name`,`db_issuer`.`employee_name` AS `issuer_name`,DATEDIFF(`db_work`.`finish_date`,`db_work`.`deadline_date`) AS `diff_finishdate`,DATEDIFF(CURDATE(),`db_work`.`deadline_date`) AS `diff_curdate` FROM `db_work` INNER JOIN `db_employee` AS `db_worker` ON `db_worker`.`employeeid` = `db_work`.`worker` INNER JOIN `db_employee` AS `db_issuer` ON `db_issuer`.`employeeid` = `db_work`.`issuer` $sqlwhere";
$result = $db->query($sql);
